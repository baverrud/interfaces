-- =====================================================================
-- axi4_slave_unified.vhd - full AXI4 slave (unified axi4_t port)
-- =====================================================================
--
-- !!! INTENTIONALLY BROKEN -- kept as a test case for future tool versions. !!!
--
-- Companion to axi4_master_unified.vhd. See that file for details.
-- =====================================================================
library ieee;
...
entity axi4_slave_unified is
  generic (DATA_W: positive:=32; ADDR_W: positive:=32; ID_W: positive:=4; DEPTH: positive:=256);
  port (aclk, aresetn: in std_logic;
      -- Unified AXI4 slave view: all 5 channels in one record
      s: view slave of axi4_t);
end entity;

architecture rtl of axi4_slave_unified is
  ...
  begin
    if aresetn = '0' then wstate <= W_IDLE; wdata_pending <= '0';
      ...
      case wstate is
        ...
        -- W_RESP: assert bvalid until master acknowledges
