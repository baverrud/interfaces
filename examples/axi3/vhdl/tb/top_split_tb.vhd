-- =====================================================================
-- top_split_tb.vhd - AXI3 split-master demo self-checking testbench
-- =====================================================================
library ieee;
...
begin
  ...
  dut : entity work.top_split
    generic map (DATA_W=>DATA_W, ADDR_W=>ADDR_W, ID_W=>ID_W,
          BURST_LEN=>BURST_LEN, DEPTH=>DEPTH, SPLIT_SLAVE=>SPLIT_SLAVE)
  ...
  begin
    ...
    report "=== AXI3 Split-Master Demo (" & integer'image(BURST_LEN) & "-beat, " &
    ...
    for i in 1 to 20 * BURST_LEN * 10 loop
      ...
      if rd_valid = '1' then
        ...
        if rd_data /= expected then
          report "Beat " & integer'image(beat_count) & " mismatch" severity error;
          errors := errors + 1;
        else
