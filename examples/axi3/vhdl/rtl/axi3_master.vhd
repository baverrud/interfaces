-- =====================================================================
-- axi3_master.vhd - full AXI3 master (per-channel ports)
-- =====================================================================
-- Drives a complete AXI3 write-then-read burst sequence autonomously.
-- AXI3-specific: awlen/arlen are 4-bit, awlock/arlock are 2-bit,
-- no awregion/arregion, wid present on write data channel.
-- =====================================================================
library ieee;
...
architecture rtl of axi3_master is
  constant AXLEN: unsigned(3 downto 0) := to_unsigned(BURST_LEN - 1, 4);  -- AXI3: 4-bit
  type state_t is (S_AW, S_WRITE, S_B, S_AR, S_READ, S_DONE);
  signal state: state_t := S_AW;
  signal beat_cnt: unsigned(3 downto 0);
begin
  -- ===================================================================
  -- Concurrent assignments: fixed AXI3 sideband signals
  -- ===================================================================
  ...
  w.wid       <= (w.wid'range => '0');  -- AXI3: write interleaving ID
