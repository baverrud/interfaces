-- =====================================================================
-- axi3_master_unified.vhd - full AXI3 master (unified axi3_t port)
-- =====================================================================
--
-- !!! INTENTIONALLY BROKEN -- kept as a test case for future tool versions. !!!
--
-- VHDL-2019 `view master of axi3_t` requires a record with more than 6
-- unconstrained std_logic_vector elements, which exceeds the parser limit
-- in both Questa 2025.3 and Vivado 2026.1.
--
-- Use `top_hp.vhd` (fully constrained `axi3_hp_t`) for a working example
...
begin
  ...
  m.awsize   <= "010"; m.awburst <= "01"; m.awlock <= "00";
  m.awcache  <= "0000"; m.awprot <= "000"; m.awqos <= "0000";
  m.awuser   <= (m.awuser'range => '0');
  m.arid     <= (m.arid'range => '0');
  m.arlen    <= std_logic_vector(AXLEN);
  m.arsize   <= "010"; m.arburst <= "01"; m.arlock <= "00";
  m.arcache  <= "0000"; m.arprot <= "000"; m.arqos <= "0000";
  ...
  begin
    if aresetn = '0' then
      ...
      case state is
        ...
        when S_WRITE =>
          if m.wvalid = '0' then
            m.wvalid <= '1'; m.wlast <= '1' when BURST_LEN = 1 else '0';
